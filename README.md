...
| kms\_key\_create | Whether or not to create a KMS key. This value cannot be computed automatically in Terraform 0.11.X. | string | `"true"` | no |
...
| security\_group\_create | Whether or not to create a Security Group for the EFS. This value cannot be computed automatically in Terraform 0.11.X. | string | `"true"` | no |
| security\_group\_ids | Security groups to be used by the EFS mount targets. Should be specified when security_group_create is false. | list | `[]` | no |
